...
void MyGLWidget::modelTransformQuadrat(glm::vec3 posicio)
{
  ...
  TG = glm::translate(TG,posicio*scale);  
  TG = glm::scale(TG,glm::vec3(0.25*scale,0.25*scale,0.25*scale));
...
void MyGLWidget::modelTransformCavall(glm::vec3 posicio)
{
  ...
  TG = glm::scale(TG,glm::vec3(0.08*scale,0.08*scale,0.08*scale)); 
...
//  glViewport (0, 0, ample, alt);
  ...
  for (int i = 0; i < 8; ++i) {
      for (int j = 0; j < 8; ++j) {
          ...
          modelTransformQuadrat(glm::vec3(-1.0+0.125+0.25*i,-1.0+0.125+0.25*j,0));
          if (blancQ) glUniform3fv(colorLoc, 1, &blancTauler[0]);
  ...
  if (comer)  glUniform3fv(colorLoc, 1, &blanc[0]);
...
void MyGLWidget::carregaShaders()
{
 ...
 colorLoc = glGetUniformLocation(program->programId(), "ColorObj");
} 
